...
do
	...
	curl "http://linserv1.cims.nyu.edu:25808/search?query=$QUERY&ranker=$RANKER&format=text" >> $FILE
	...
	curl "http://linserv1.cims.nyu.edu:25808/search?query="$QUERY"&ranker="$RANKER"&format=text" | java edu.nyu.cs.cs2580.Evaluator $JUDGEMENTS >> $FILE
